...
export class EnvironmentValidationService {
  ...
  private validateProductionEnvironment(errors: string[]): void {
    ...
    const jwtSecret = this.configService.get<string>('JWT_SECRET');
    if (jwtSecret && (jwtSecret.includes('development') || jwtSecret.includes('test'))) {
      errors.push('JWT_SECRET should not contain development/test keywords in production');
    }
  ...
  getConfig(): EnvironmentConfig {
    return {
      ...
      mpEnvironment: this.configService.get<string>('MP_ENVIRONMENT', 'sandbox'),
      ...
      storageRegion: this.configService.get<string>('STORAGE_REGION', 'us-east-1'),
