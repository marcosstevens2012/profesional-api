...
export function validateEnvironment(): EnvironmentVariables {
  try {
    ...
    if (validated.NODE_ENV === 'production') {
      if (validated.JWT_SECRET.includes('development') || validated.JWT_SECRET.includes('test')) {
        throw new Error('JWT_SECRET should not contain development/test keywords in production');
      }

      if (validated.MP_ACCESS_TOKEN.startsWith('TEST-')) {
        throw new Error('MP_ACCESS_TOKEN should not be a test token in production');
      }
    }
